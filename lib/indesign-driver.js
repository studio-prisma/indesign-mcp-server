...
// UndoModes.ENTIRE_SCRIPT. Read out of InDesign 21.5 rather than guessed -
// the four UndoModes members sit in one numeric block and are easy to confuse.
 ...
 */
function buildPowerShellRunner(jsxPath, undoName) {
  ...
  return [
    ...
    // Five arguments, not two. withArguments has to be an empty array:
    ...
    undoName === null
      ...
      : `$app.DoScript($script, ${ID_JAVASCRIPT}, @(), ${UNDO_ENTIRE_SCRIPT}, '${label}') | Out-Null`,
    'exit 0',
  ].join('\r\n');
}
...
function buildAppleScriptRunner(jsxPath, undoName) {
...
export async function executeInDesignScript(rawScript, options = {}) {
  ...
  try {
    ...
    if (IS_WIN) {
      fs.writeFileSync(runnerPath, buildPowerShellRunner(jsxPath, undoName), 'utf8');
