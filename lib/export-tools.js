...
/**
 * IDML - the interchange format.
 *
 * The one export that is not about the finished job: IDML opens in older
 * InDesign versions and in other tools, which makes it the way to hand a
 * document to somebody who cannot open the .indd. It is also the only
 * lossless text-based form of a layout, so it is what belongs in version
 * control if a layout ever has to go there.
 *
 * Not reachable through call_method: exportFile needs a File object, and the
 * generic layer passes data, never constructed objects.
 */
export function exportIDML({ filePath }) {
  const validated = validateFilePath(filePath);
  return `
    if (app.documents.length === 0) {
      __result__ = "No document open.";
      ...
      try {
        ...
        if (!f.exists) {
          ...
          __result__ = "Exported IDML to " + f.fsName +
            " | " + Math.round(f.length / 1024) + " KB" +
            " | opens in InDesign CS4 and newer";
        }
      } catch (e) {
        __result__ = "ERROR exporting IDML: " + e.message;
      }
    }
  `;
}
