...
// UndoModes members. Read out of InDesign 21.5 rather than guessed - the four
// sit in one numeric block and are easy to confuse.
//
// ENTIRE_SCRIPT groups a call into one undo step. SCRIPT_REQUEST is the
// ungrouped mode, and it is the only one of the four under which doc.undo()
// still works: AUTO_UNDO, FAST_ENTIRE_SCRIPT and ENTIRE_SCRIPT all make
// InDesign refuse with "the last command cannot be undone".
const UNDO_ENTIRE_SCRIPT = 1699963733;
const UNDO_SCRIPT_REQUEST = 1699967573;

// Without an undo mode, DoScript leaves one history entry per internal
// operation, labelled by InDesign in the interface language: undoing a single
// tool call then means clicking undo an unknown number of times, past entries
// reading "Resize" or "Grosse andern". With ENTIRE_SCRIPT plus a name, one
// call is one step and the history says which tool made it.
//
// The label travels in async-local storage rather than through 83 call sites.
// AsyncLocalStorage, not a module variable, so concurrent requests cannot swap
// each other's labels.
const undoLabel = new AsyncLocalStorage();

/**
 * Run fn with every script it executes grouped under one undo step of this
 * name. The MCP server wraps each tool call in this.
 */
export function withUndoLabel(label, fn) {
  return undoLabel.run(safeUndoLabel(label), fn);
}

/**
 * The label reaches a PowerShell string literal and an AppleScript one. It
 * comes from the tool name, never from caller input, and this keeps it that
 * way: anything outside the character set is dropped rather than escaped.
 */
function safeUndoLabel(label) {
  const clean = String(label ?? '').replace(/[^A-Za-z0-9 _.-]/g, '').slice(0, 60);
  return clean || 'MCP script';
}
...
/**
 * Write a text file with a UTF-8 BOM.
 *
 * Both consumers need it. Windows PowerShell 5.1 reads a BOM-less file as the
 * ANSI codepage, so on a German system a UTF-8 "Ü" in the runner arrives as
 * "Ãœ" - two characters, charCodes 195 and 339. ExtendScript makes the same
 * assumption about a .jsx without a BOM; it happens to auto-detect UTF-8 in
 * InDesign 21, but that is a heuristic, not a promise.
 */
export function writeFileWithBom(filePath, text) {
  fs.writeFileSync(filePath, '﻿' + text, 'utf8');
}

/** Drop a leading U+FEFF, whether it arrived as a BOM or as a character. */
 ...
 */
export function buildPowerShellRunner(jsxPath, undoName) {
  const p = psSingleQuote(jsxPath);
  const label = psSingleQuote(safeUndoLabel(undoName));
  // undoName === null means: do not group. The undo tool needs that - calling
  // doc.undo() from inside a script that is itself being recorded as one undo
  // step is refused by InDesign ("the last command cannot be undone").
  const undoMode = undoName === null ? UNDO_SCRIPT_REQUEST : UNDO_ENTIRE_SCRIPT;
  const progIds = WIN_PROGIDS.map((id) => `'${psSingleQuote(id)}'`).join(', ');
  return [
    // Windows PowerShell 5.1 writes stdout and stderr in the console's OEM
    // codepage - CP850 on a German system - while this process decodes them as
    // UTF-8. Every umlaut in an InDesign error message would arrive as U+FFFD,
    // and explainFailure() matches on German wording.
    ...
    // Always five arguments, never the two-argument overload: one shape for
    // every call means COM overload resolution cannot differ between the
    // grouped and the ungrouped path. withArguments has to be an empty array -
    // passing $null there raises a NullReferenceException inside the COM
    // interop before InDesign ever sees the call.
    `$app.DoScript($script, ${ID_JAVASCRIPT}, @(), ${undoMode}, '${label}') | Out-Null`,
    'exit 0',
  ].join('\r\n');
}
...
export function buildAppleScriptRunner(jsxPath, undoName) {
  const p = asDoubleQuote(jsxPath);
  const label = asDoubleQuote(safeUndoLabel(undoName));
  ...
  const attempts = MAC_APP_NAMES.map((name) => `
    try
      tell application "${asDoubleQuote(name)}"
        activate
        do script (POSIX file "${p}") language javascript${undoName === null ? '' : ` with undo mode entire script undo name "${label}"`}
      end tell
      return "ok"
    end try`).join('\n');
...
export async function executeInDesignScript(rawScript, options = {}) {
  ...
  // null is meaningful here: it switches undo grouping off for this call.
  const undoName = options.undoName === null
    ? null
    : (options.undoName ?? undoLabel.getStore());
  ...
  try {
    writeFileWithBom(jsxPath, wrapped);

    if (IS_WIN) {
      writeFileWithBom(runnerPath, buildPowerShellRunner(jsxPath, undoName));
      runPowerShellFile(runnerPath, timeout);
    } else if (IS_MAC) {
      // osascript reads UTF-8 without a BOM; a BOM would be a syntax error.
      fs.writeFileSync(runnerPath, buildAppleScriptRunner(jsxPath, undoName), 'utf8');
    ...
    if (fs.existsSync(resultPath)) {
      // Whether ExtendScript puts a BOM in front of a file it wrote with
      // encoding "UTF-8" varies by version. A stray U+FEFF at the head of a
      // tool result is invisible in a terminal but breaks string comparison.
      return stripBom(fs.readFileSync(resultPath, 'utf8'));
    }
