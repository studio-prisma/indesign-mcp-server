 ...
 *
 * Every document has one section covering all pages, whether anybody asked
 * for it or not. A second one is what makes front matter number i, ii, iii
 * while the body starts again at 1 - the ordinary shape of any document with
 * a title page, and not reachable through the generic layer, because adding a
 * section is a method call on a collection.
 *
 * These members were read out of InDesign 21.5. KATAKANA_MODERN and
 * FULL_WIDTH_ARABIC are not among them, though both appear in older
 * references.
 */
export const PAGE_NUMBER_STYLES = [
  'ARABIC', 'LOWER_ROMAN', 'UPPER_ROMAN', 'LOWER_LETTERS', 'UPPER_LETTERS',
  'KANJI', 'SINGLE_LEADING_ZEROS', 'DOUBLE_LEADING_ZEROS', 'TRIPLE_LEADING_ZEROS',
];

export function createSection({
  pageIndex = 0, continueNumbering = false, pageNumberStart = 1,
  pageNumberStyle = 'ARABIC', sectionPrefix, includeSectionPrefix = false,
  marker,
}) {
  const pi = index(pageIndex, { name: 'pageIndex' });
  const style = enumOf(pageNumberStyle, PAGE_NUMBER_STYLES, { name: 'pageNumberStyle' });
  const start = num(pageNumberStart, { name: 'pageNumberStart', min: 1, max: 99999 });
  ...
  return `
    if (app.documents.length === 0) {
      ...
      var doc = app.activeDocument;
      try {
        if (${pi} >= doc.pages.length) {
          __result__ = "No page " + (${pi} + 1) + " - the document has " +
            doc.pages.length + ".";
        } else {
          var page = doc.pages[${pi}];
          var existing = null;
          for (var i = 0; i < doc.sections.length; i++) {
            if (doc.sections[i].pageStart === page) { existing = doc.sections[i]; break; }
          }
          var sec = existing !== null ? existing : doc.sections.add(page);
          sec.continueNumbering = ${bool(continueNumbering, { name: 'continueNumbering' })};
          ${continueNumbering ? '' : `sec.pageNumberStart = ${start};`}
          sec.pageNumberStyle = PageNumberStyle.${style};
          ${sectionPrefix === undefined ? '' : `sec.sectionPrefix = ${str(sectionPrefix)};`}
          sec.includeSectionPrefix = ${bool(includeSectionPrefix, { name: 'includeSectionPrefix' })};
          ${marker === undefined ? '' : `sec.marker = ${str(marker)};`}

          // Report the page names, because that is the thing being changed -
          // the numbering, not the section object.
          var names = [];
          for (var n = 0; n < Math.min(doc.pages.length, 12); n++) {
            names.push(doc.pages[n].name);
          }
          __result__ = (existing !== null ? "Updated the section" : "New section") +
            " starting on the " + (${pi} + 1) + ". page of the document" +
            " | ${style}" +
            (${continueNumbering ? 'true' : 'false'} ? ", continuing the previous count" : ", starting at " + ${start}) +
            "\\nPage names now: " + names.join(", ") +
            (doc.pages.length > 12 ? ", ..." : "") +
            "\\nSections in the document: " + doc.sections.length;
        }
      } catch (e) {
        __result__ = "ERROR creating section: " + e.message;
      }
    }
  `;
}
...
export function listSections() {
  return `
    if (app.documents.length === 0) {
      ...
      var doc = app.activeDocument;
      try {
        var out = doc.sections.length + " section(s):";
        for (var i = 0; i < doc.sections.length; i++) {
          var s = doc.sections[i];
          var startsOn = "?";
          for (var p = 0; p < doc.pages.length; p++) {
            if (doc.pages[p] === s.pageStart) { startsOn = String(p); break; }
          }
          out += "\\n  [" + i + "] page index " + startsOn +
            " | name '" + s.pageStart.name + "'" +
            " | " + String(s.pageNumberStyle).replace("PageNumberStyle.", "") +
            (s.continueNumbering ? " | continues" : " | starts at " + s.pageNumberStart) +
            (s.sectionPrefix !== "" ? " | prefix '" + s.sectionPrefix + "'" : "") +
            (s.marker !== "" ? " | marker '" + s.marker + "'" : "");
        }
        __result__ = out;
      } catch (e) {
        __result__ = "ERROR listing sections: " + e.message;
      }
    }
  `;
}
